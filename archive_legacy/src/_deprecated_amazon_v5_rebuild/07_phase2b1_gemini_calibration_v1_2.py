...
def call_gemini(model, prompt, is_dry_run=False, b_df=None, is_repair=False):
    ...
    if is_dry_run:
        ...
        # We inject specific errors based on the first ID in the batch to test the validator
        first_id = b_df.iloc[0]['annotation_item_id']
        items = []
        for i, row in b_df.iterrows():
            iid = row['annotation_item_id']
            text = str(row.get('reviewText', ''))
            
            # Determine fixture based on index in batch
            # We will use the first 4 items in preflight to test specific failures
            # Item 0: Valid
            # Item 1: Missing ID (we just skip it)
            # Item 2: Non-existent quote
            # Item 3: Duplicate quote
            
            
            if not is_repair and 'batch_0_preflight' in prompt:
            ...
            item = {
                ...
                "Evidence1": text[:15] if len(text) > 15 else text, # usually valid
                "Aspect2": None,
                "Polarity2": None,
        ...
        latency = 0.5
        return json.dumps({"items": items}), latency, "SUCCESS"
...
def run_phase2b1(is_dry_run=False):
    print(f"Starting Phase 2B-1: Gemini API Quota-Aware Batching v1.2 (Dry Run: {is_dry_run})")
    if not is_dry_run:
        setup_gemini()
    ...
    ckpt_path = base_dir / ("gemini_batch_run_checkpoint_v1_2" + ("_dryrun" if is_dry_run else "") + ".json")
    if ckpt_path.exists():
    ...
    if is_dry_run:
        ...
        with open(base_dir / "gemini_request_budget_v1_1.json", "w") as f:
    ...
    if len(all_res) > 0 and is_dry_run:
        ...
        with open(base_dir / "gemini_batch_dry_run_report_v1_1.md", "w") as f:
            f.write("# Dry Run Report v1.1\n\n")
