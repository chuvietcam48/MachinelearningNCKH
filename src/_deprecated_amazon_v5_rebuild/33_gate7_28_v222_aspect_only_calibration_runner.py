...
POLICY_VERSION = "semantic_policy_v2_2_2_aspect_only_atomic"
CAMPAIGN_ID = "V222ASPECTONLYCALIBRATION_FRESH_V1"
...
POLICY_PATH = OUT_DIR / "semantic_policy_v2_2_2_aspect_only_atomic.md"
LOCKED_HASHES = {
...
def normalize_targets(row: dict[str, str], targets: list[dict[str, str]]) -> list[dict[str, str]]:
    ...
    prior_expectation = any(
        ...
        for cue in (
            ...
            "expected to hear",
            "not the same",
            "confused if it's the same series",
        )
    )
    ...
    for target in targets:
        ...
        if aspect == "Listing_Expectation_Compatibility" and not explicit_listing and (prior_expectation or digital_or_format_preference):
...
def assemble(row: dict[str, str], targets: list[dict[str, str]]) -> dict[str, Any]:
    targets = normalize_targets(row, targets)
    deduped = []
    seen = set()
    for target in targets:
        key = (target["aspect"], target["polarity"])
...
- Broken, cracked, damaged, defective, or falling-apart CD cases, jewel cases, discs, vinyl, playable media, or item components are Product_Condition_Quality, not Packaging_Presentation.
- Price paid, shipping cost, not worth the money, not worth shipping, bargain-bin, too expensive, or value-for-money complaint is Price_Value, not Packaging_Presentation.
- Packaging_Presentation covers only outer packaging, booklet, liner notes, cover/back-cover metadata, box/cardboard, visual presentation, or missing/poor presentation information.
