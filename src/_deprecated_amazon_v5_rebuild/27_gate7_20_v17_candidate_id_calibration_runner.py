#!/usr/bin/env python
"""Gate 7.20 v1.7 candidate-ID calibration runner.

v1.7 keeps the candidate-ID evidence architecture and applies conservative
general semantic normalization rules learned from Gate 7 replay failures.
"""
...
def normalize_v17(item: dict[str, Any]) -> None:
    BASE_NORMALIZE(item)
    text = text_of(item)
    ...
    # Where the provider converted prior-series or edited-mix expectation into a
    # negative domain slot, keep a true mixed domain result if independent praise exists.
    if has_aspect(item, "Domain_Experience", "Negative") and positive_domain_cue and item.get("Aspect2") is None:
    ...
    for slot in ("1", "2"):
        if item.get(f"Aspect{slot}") == "Product_Condition_Quality" and not technical_quality_cue and not physical_condition_cue:
            item[f"Aspect{slot}"] = "Domain_Experience"
    if technical_quality_cue and not has_aspect(item, "Product_Condition_Quality"):
        if item.get("Aspect2") is None and item.get("Aspect1") != "None":
            polarity = "Negative" if any(cue in text for cue in ("limited frequency", "hard to hear", "background noise", "mediocre")) else "Positive"
            copy_slot(item, "1", "2", "Product_Condition_Quality", polarity)
    ...
    if any(cue in text for cue in ("booklet", "album covers", "back of their respective", "don't list artists", "song titles")):
        if "sound is very good" in text or "sound quality" in text:
            set_slot(item, "1", "Product_Condition_Quality", "Positive", item.get("Evidence_Source1"), item.get("Evidence1"))
            if item.get("Aspect2") is None:
                copy_slot(item, "1", "2", "Packaging_Presentation", "Negative")
            else:
                set_slot(item, "2", "Packaging_Presentation", "Negative")
...
def configure_runner() -> None:
    runner.POLICY_VERSION = "semantic_policy_v1_7_candidate_id"
    runner.CAMPAIGN_ID = "V17CIDCALIBRATION_FRESH_V1"
    ...
    runner.apply_policy_normalization = normalize_v17
