#!/usr/bin/env python
"""Gate 7.30 v2.2.4 aspect-only calibration runner.
...
Includes v2.2.4 pipeline bug fixes.
"""
...
POLICY_VERSION = "semantic_policy_v2_2_4_aspect_only_atomic"
CAMPAIGN_ID = "V224ASPECTONLYCALIBRATION_FRESH_V1"

v222.POLICY_VERSION = POLICY_VERSION
v222.CAMPAIGN_ID = CAMPAIGN_ID
v222.POLICY_PATH = v222.OUT_DIR / "semantic_policy_v2_2_4_aspect_only_atomic.md"
...
- Product_Condition_Quality covers physical item defects, damaged playable media/case, AND technical sound/recording/mastering quality (e.g., sound quality, recording quality, recorded sound, remastering, mix, frequency range, hard to hear, background noise, sounding, sounds better).
- IMPORTANT: If the review discusses technical sound quality (sound quality, recording quality, mix, background noise, hard to hear, remastering, sounding, sounds better), MUST output Product_Condition_Quality as an independent target, do not swallow it into Domain_Experience.
...
- CRITICAL: If the review explicitly complains about 'pricey', 'expensive', or 'paying full album price', you MUST extract Price_Value Negative. This is a critical churn signal. Prioritize extracting Price_Value Negative even if you have to drop a secondary Domain_Experience target due to target limits.
...
def normalize_targets(row: dict[str, str], targets: list[dict[str, str]]) -> list[dict[str, str]]:
    ...
    # RULE 3: Same-aspect mixed Domain handling
...
def compare_against_human(repo: Path, run_dir: Path) -> dict[str, Any]:
    ...
    if (aspect_agree_total >= 85 and
        ...
        status = "V224_ASPECT_ONLY_CALIBRATION_PASS"
    else:
        status = "V224_ASPECT_ONLY_CALIBRATION_REVIEW_REQUIRED"
