...
def normalize_v20(item: dict[str, Any]) -> None:
    BASE_NORMALIZE(item)
    text = text_of(item)
    ...
    # Prior-series, availability, edited-length, or format-preference complaints
    # are domain expectations unless the listing or delivered item is wrong.
    for slot in ("1", "2"):
        if item.get(f"Aspect{slot}") == "Listing_Expectation_Compatibility" and not explicit_listing_cue:
            if any(cue in text for cue in ("booklet", "album covers", "back of their respective", "don't list artists", "song titles")):
                item[f"Aspect{slot}"] = "Packaging_Presentation"
            else:
                item[f"Aspect{slot}"] = "Domain_Experience"

    # Broken jewel/CD cases are product condition, except when the text
    # foregrounds the money/shipping value complaint as the second complaint.
    for slot in ("1", "2"):
    ...
    for slot in ("1", "2"):
        creative_style_quality_overcount = any(
            cue in text
            for cue in (
        ...
        if item.get(f"Aspect{slot}") == "Product_Condition_Quality" and creative_style_quality_overcount and not technical_quality_cue and not physical_condition_cue:
            item[f"Aspect{slot}"] = "Domain_Experience"
    ...
    if item.get("Polarity2") == "Positive" and item.get("Aspect1") == "Domain_Experience" and item.get("Polarity1") == "Positive":
        if item.get("Aspect2") in {"Other_Specific", "Price_Value", "Listing_Expectation_Compatibility"}:
            drop_slot2(item)
        elif item.get("Aspect2") == "Product_Condition_Quality" and any(cue in text for cue in ("raw production style", "passion and originality", "pretending to be")):
            drop_slot2(item)
    ...
    if item.get("Aspect1") == "Product_Condition_Quality" and item.get("Polarity1") == "Positive" and positive_domain_cue and any(cue in text for cue in ("raw production style", "passion and originality", "pretending to be")):
        item["Aspect1"] = "Domain_Experience"
...
def configure_runner() -> None:
    runner.POLICY_VERSION = "semantic_policy_v2_0_candidate_id"
    runner.CAMPAIGN_ID = "V20CIDCALIBRATION_FRESH_V1"
    ...
    runner.apply_policy_normalization = normalize_v20
