...
def main():
    ...
    if 'semantic_available' in df_train.columns:
        sem_train = df_train[df_train['semantic_available'] == 1]
        sem_train.to_parquet(out_dir / "master_train_semantic.parquet", index=False)
        df_val[df_val['semantic_available'] == 1].to_parquet(out_dir / "master_val_semantic.parquet", index=False)
        df_test[df_test['semantic_available'] == 1].to_parquet(out_dir / "master_test_semantic.parquet", index=False)
