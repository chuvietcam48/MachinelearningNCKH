...
def run_phase2b1(is_dry_run=False):
    print(f"Starting Phase 2B-1: Gemini API Quota-Aware Batching v1.3 (Dry Run: {is_dry_run})")
    if not is_dry_run:
        setup_gemini()
    ...
    with open(base_dir / "gemini_semantic_prompt_template_v1_2.md", "r") as f:
        prompt_template = f.read()
    with open(base_dir / "taxonomy_v1_1.md", "r") as f:
        taxonomy_text = f.read()
    with open(base_dir / "semantic_output_schema_v1_2.json", "r") as f:
        schema_text = f.read()
    ...
    ckpt_path = base_dir / ("gemini_batch_run_checkpoint_v1_3" + ("_dryrun" if is_dry_run else "") + ".json")
    if ckpt_path.exists():
    ...
    if is_dry_run:
        ...
        with open(base_dir / "gemini_request_budget_v1_3.json", "w") as f:
    ...
    if len(all_res) > 0 and is_dry_run:
        ...
        with open(base_dir / "gemini_batch_dry_run_report_v1_3.md", "w") as f:
            f.write("# Dry Run Report v1.3\n\n")
...
"""
        ...
        if valid_res:
            pd.DataFrame([{
                'annotation_item_id': r['annotation_item_id'],
                **r['parsed_json']
            } for r in valid_res]).to_parquet(base_dir / "llm_calibration_labels_v1_3.parquet", index=False)
            
        if invalid_res:
            pd.DataFrame([{
                'annotation_item_id': r['annotation_item_id'],
                'errors': str(r['errors'])
            } for r in invalid_res]).to_csv(base_dir / "llm_calibration_invalid_queue_v1_3.csv", index=False)
        else:
            pd.DataFrame(columns=['annotation_item_id', 'errors']).to_csv(base_dir / "llm_calibration_invalid_queue_v1_3.csv", index=False)
            
        pd.DataFrame([{
            'annotation_item_id': r['annotation_item_id'],
            'is_valid': r['is_valid'],
            'errors': str(r['errors'])
        } for r in calib_res]).to_csv(base_dir / "llm_calibration_validation_v1_3.csv", index=False)
        ...
        with open(base_dir / "gemini_call_manifest_v1_3.json", "w") as f:
            json.dump(manifest, f, indent=2)
        with open(base_dir / "llm_calibration_provenance_v1_3.json", "w") as f:
            json.dump(manifest, f, indent=2)
            
        report = f"""# Phase 2B-1: LLM-assisted silver calibration labels Report v1.3
...
**Disclaimer:** These metrics represent structural processing and raw distribution only. No claim is made regarding human verification, ground facts, or utility for downstream tasks.
"""
        with open(base_dir / "phase2b1_llm_calibration_report_v1_3.md", "w") as f:
            f.write(report)
