...
$approvalFile = Join-Path $approvalDir "live_approval_V12EXP_PRIMARY_V3_P000001_P000010_A004.json"
...
$approval = [ordered]@{
    ...
    approval_id = "LIVE_APPROVAL_V12EXP_PRIMARY_V3_P000001_P000010_A004_OPERATOR"
    ...
    operator_note = "A004 uses serving-light required-field provider schema after A003 returned empty item objects; strict local validation remains unchanged."
}

if (Test-Path -LiteralPath $approvalFile) {
    throw "Approval file already exists; create the next attempt ID instead of reusing A004: $approvalFile"
}
